...
async function loadFlowPoints() {
  ...
  Microsoft.Maps.Events.addHandler(clusterLayer, "mouseover", (e) => {
    ...
    for (const pin of pins) {
      ...
      lane.averageSpeed += data.AverageVehicleSpeed * data.VehicleFlowRate;
      ...
      type.averageSpeed += data.AverageVehicleSpeed * data.VehicleFlowRate;
  ...
  Microsoft.Maps.Events.addHandler(clusterLayer, "click", (e) => {
    ...
    const data = pins[0]["data"] as TrafikVerketTrafficFlow;

    const location = data.Geometry.Point;
    const radius = 5;

    onGraphOpen(location, radius);
    showGraph();
  });
}
